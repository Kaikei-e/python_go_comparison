...
- テストを書くことでソースの質が上がる。事前にバグを見つけることができ、未知のバグや原因不明のバグが減る。
  ...
  - Pythonにも標準のテストモジュールは存在する。大きくわけて3つの段階のものが存在する。ドキュメントに書き込むもの、より一般的な、Goのように別ファイルに記載し実行するもの、フレームワークのpytestなどを利用したもの

### 型付けの強み
- 静的型付け言語は極論すれば、ソースがよりドキュメントに近くなりえて、ソースの可読性やバグ追跡のしやすさも向上する。